...
// Added for this project's go2_nav integration (not upstream). gz-sim's gpu_lidar
// sensor (see go2_description/xacro/gazebo.xacro) only ever publishes x, y, z,
// intensity on its bridged PointCloud2 -- no ring, no per-point time/t field, a
// hard limitation of that sensor type, not a config oversight. Reusing VELO16/
// OUST64/KMOUST64 here would call pcl::fromROSMsg into point structs expecting
// fields that don't exist on the cloud, which silently zero-fills them instead of
// erroring -- producing garbage/empty clouds rather than a clean fallback.
// pcl::PointXYZI's fields (x, y, z, intensity) all genuinely exist on this cloud,
// so this is a real, safe field match.
//
// imu_processing.hpp's backward-propagation deskew reads each point's curvature
// (repurposed as an intra-scan time offset in ms) to interpolate IMU pose during
// the scan. There's no real per-point time to read here, but curvature can't
// just be left at 0 for every point either: spark_fast_lio.cpp's Sync() (the
// meas.lidar->points.back().curvature check) uses the LAST point's curvature as
// the measured scan duration to maintain its own running lidar_mean_scantime_
// estimate, and a constant 0 there is a degenerate value that estimate never
// recovers from -- confirmed live: every scan then computed a zero-length
// [lidar_beg_time, lidar_end_time] window, imu_processing.hpp's undistortion
// found no points inside that window, and every single scan was silently
// dropped ("No point, skip this scan!" logged continuously, /robot1/odom never
// published). Synthesizing curvature as a linear ramp across the point index,
// spanning the sensor's real inter-scan period (1000/SCAN_RATE ms, SCAN_RATE
// set from preprocess.scan_rate to match gazebo.xacro's gpu_lidar update_rate),
// fixes this: it's not a real per-point timestamp (gz-sim's gpu_lidar renders a
// whole scan in one simulation step, not a true point-by-point sweep), but it
// gives the pipeline a consistent, correctly-scaled scan duration to key off of,
// and biases deskew correction toward the point order the sensor actually
// outputs in -- a reasonable proxy given no better information exists, and
// confirmed live to make points flow again (see fast_lio.launch.py's module
// docstring for the end-to-end verification).
void Preprocess::generic_handler(const sensor_msgs::msg::PointCloud2 &msg) {
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  pl_from_pilots.clear();
  ...
  const size_t plsize        = pl_orig.points.size();
  const float scan_period_ms = 1000.f / static_cast<float>(SCAN_RATE);
  ...
  for (size_t i = 0; i < plsize; i++) {
    ...
    added_pt.normal_z  = 0;
    added_pt.curvature =
        plsize > 1 ? (static_cast<float>(i) / static_cast<float>(plsize - 1)) * scan_period_ms
                   : 0.0f;
