...
// Number of raw IMU samples averaged to determine the initial gravity
// direction and gyro bias (see IMU_init() below). Was 10 -- at this sensor's
// 100Hz IMU rate that's only 0.1s, which is nowhere near enough to average
// out a legged robot's spawn-settling transient or its gait controller's
// continuous balance micro-adjustments (neither of which is "stationary" in
// the way this code assumes). A corrupted average here bakes a permanently
// wrong "down" direction into the whole filter: confirmed via live testing
// -- FAST-LIO's estimated orientation ended up tilted ~34 degrees off level
// while the real robot (gait odometry) was flat, and its position ran away
// to thousands of metres within minutes, consistent with gravity never
// being fully cancelled out of the accelerometer reading and getting
// double-integrated forever. Confirmed 200 (~2s) already fixes the runaway
// growth (pose held rock-steady over 30s stationary, vs thousands of
// metres before), but left a smaller fixed residual bias (~1.2m position,
// ~19 degrees tilt) that varied between runs -- consistent with sample
// noise not yet fully averaged out, not a systematic bug. Bumped further to
// test that theory.
