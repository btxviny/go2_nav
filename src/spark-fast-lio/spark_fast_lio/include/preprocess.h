...
// GENERIC = 5 added for this project: a plain sensor_msgs/PointCloud2 with only
// x/y/z/intensity fields (no ring, no per-point time) -- see generic_handler()
// in preprocess.cpp for why none of the other types can be reused as-is.
enum LID_TYPE { AVIA = 1, VELO16 = 2, OUST64 = 3, KMOUST64 = 4, GENERIC = 5 };  // {1, 2, 3, 4, 5}
...
class Preprocess {
 ...
 private:
  ...
  void generic_handler(const sensor_msgs::msg::PointCloud2 &msg);
