...
#
# Usage: ~/go2_nav/src/go2_office_sim/scripts/run_stack.sh
# Ctrl+C stops everything this script started.

set -u

# "ros2 launch go2_office_sim", not bare "go2_office_sim": this script's own
# path (.../scripts/run_stack.sh) contains the substring "go2_office_sim" too
# -- pkill/pgrep only ever auto-exclude their own PID, not an ancestor script
# that invoked them, so a bare "go2_office_sim" term matched and killed this
...
PROJECT_PROCS="gz sim|ros_gz_bridge|quadropted_controller/lib|ros2 launch go2_office_sim|kiss_icp_node|rviz2|slam_toolbox|controller_server|planner_server|behavior_server|bt_navigator|waypoint_follower|velocity_smoother|collision_monitor|opennav_docking|route_server|smoother_server|pointcloud_to_laserscan_node|lifecycle_manager"
