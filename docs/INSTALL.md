...
sudo apt install -y \
  ...
  ros-jazzy-ros2launch ros-jazzy-ros2run ros-jazzy-camera-info-manager \
...
and controller packages — `src/go2_description`, `src/quadruped_controller`,
`src/quadruped_msgs` — are not; their history is merged directly into this repo via
`git subtree`, so they come along with a plain clone.) If you forgot the flag:
